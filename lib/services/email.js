...
/**
 * Send voting threshold reached notification
 * @param {string} email - User email
 * @param {string} firstName - User first name
 * @param {Object} thresholdInfo - Threshold information
 * @returns {Promise<{success: boolean, error?: any}>}
 */
export async function sendVotingThresholdEmail(
  email,
  firstName,
  thresholdInfo,
) {
  const subject = `🎯 Voting Milestone: ${thresholdInfo.ideaTitle} Reached ${thresholdInfo.votes} Votes!`;

  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      <h1 style="color: #2563eb;">Voting Milestone Achieved!</h1>
      <p>Congratulations ${firstName}! Your idea "${thresholdInfo.ideaTitle}" has reached ${thresholdInfo.votes} votes!</p>

      <div style="background: #f8fafc; padding: 20px; border-radius: 8px; margin: 20px 0;">
        <h3 style="margin-top: 0;">Milestone Details</h3>
        <p><strong>Idea:</strong> ${thresholdInfo.ideaTitle}</p>
        <p><strong>Votes Received:</strong> ${thresholdInfo.votes}</p>
        <p><strong>Next Milestone:</strong> ${thresholdInfo.nextThreshold} votes</p>
        <p><strong>Reward Earned:</strong> ${thresholdInfo.rewardCredits} credits</p>
      </div>

      <p>Keep engaging with the community and watch your idea gain more traction!</p>

      <div style="text-align: center; margin: 30px 0;">
        <a href="${process.env.APP_URL || "http://localhost:4000"}/idea/${thresholdInfo.ideaId}" style="background: #2563eb; color: white; padding: 12px 24px; text-decoration: none; border-radius: 6px; display: inline-block;">
          View Your Idea
        </a>
      </div>
 ...
 * @param {string} email - User email
 * @param {string} firstName - User first name
 * @param {Object} rewardInfo - Reward information
 * @returns {Promise<{success: boolean, error?: any}>}
 */
export async function sendRewardDistributionEmail(
  email,
  firstName,
  rewardInfo,
) {
  const subject = `🎁 Reward Distributed: ${rewardInfo.totalCredits} Credits Added to Your Account!`;

  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      <h1 style="color: #2563eb;">Rewards Distributed!</h1>
      <p>Great news ${firstName}! You've earned credits through community participation.</p>

      <div style="background: #f8fafc; padding: 20px; border-radius: 8px; margin: 20px 0;">
        <h3 style="margin-top: 0;">Reward Summary</h3>
        <p><strong>Total Credits Earned:</strong> ${rewardInfo.totalCredits}</p>
        <p><strong>New Balance:</strong> ${rewardInfo.newBalance} credits</p>
        ...
        <h4>Earned From:</h4>
        <ul>
          ${rewardInfo.breakdown.map((item) => `<li>${item.description}: ${item.credits} credits</li>`).join("")}
        </ul>
        `
            : ""
        }
      ...
      <p>Use these credits to generate AI content, create reports, and unlock premium features!</p>
