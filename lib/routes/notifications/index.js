...
// DELETE /api/notifications/delete-unread - Delete all unread notifications
router.delete("/delete-unread", requireAuth, async (req, res) => {
  try {
    const userId = req.user.id;
    const { createClient } = await import("@supabase/supabase-js");
    const config = (await import("../../config.js")).default;
    ...
    if (error) {
      logger.error("Error deleting unread notifications:", error);
      return res.status(500).json({
        success: false,
        error: "Failed to delete unread notifications",
      });
    }

    res.json({
      success: true,
      message: "Unread notifications deleted successfully",
    });
  } catch (error) {
    logger.error("Delete unread notifications error:", error);
    res.status(500).json({
      success: false,
      error: "Internal server error",
    });
...
// Voting & Rewards Dashboard - MOVED TO MAIN ROUTES
