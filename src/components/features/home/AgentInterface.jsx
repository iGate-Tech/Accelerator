...
const AgentInterface = (props) => {
    const LogoIcon = ({ className, fillColor, ...props }) => {
        ...
        return (
<svg class={className} viewBox="130 245 230 230" width="20" height="20" {...props}>
  <g>
    <g>
    ...
    const isLoading = () => {
        // Don't block loading if no agentStore (no project yet)
        if (!props.tasksList) return true;
        return false;
    };
    ...
    const {user, isAuthenticated} = userContext || { user: () => null, isAuthenticated: () => false };
    ...
    return (
        <div id="agentBox"
            class={`${props.agentBoxClass()}`}
            style={(props.startPressed && props.startPressed()) || (props.tasksList && props.tasksList().length > 0) || (props.currentProjectId && props.currentProjectId())
                ? "position: fixed !important; bottom: 10px !important; z-index: 50 !important;" : ""}>

              <div id="agentContent"
                  class={props.agentContentClass ? props.agentContentClass() : ""}>

                  <div class="flex flex-col gap-4  px-12">
                     <Show when={isAuthenticated() && !(props.startPressed && props.startPressed()) && !(props.tasksList && props.tasksList().length > 0) && !(props.currentProjectId && props.currentProjectId())}>
                        <div ref={greetingRef}
                            id="greetingDiv"
                            ...
                            <h1 class="text-lg sm:text-2xl md:text-3xl font-sans font-light mb-2 sm:mb-2">
                                ...
                                <span class="" style="color:#00a7e0">{userName()}</span>
                    ...
                    <div class=" card overflow-visible">
                        <div class="card-body relative p-0 !gap-0 overflow-visible">
                              ...
                              <Show when={(uiState() === 'processing') && props.streamingContent && props.streamingContent().trim()}>
                              ...
                              <div class="btnshadow p-[1px] overflow-visible">
                                   <form id="taskForm" class="p-4  bg-base-100 border border-base-200 rounded-box overflow-visible">
                                      ...
                                      rows="1"
                                          name="prompt"
                                          id="promptTextarea"
                                          class={`text-base-content text-lg w-full focus:ring-0 active:ring-0 ${uiState() === 'processing' ? 'opacity-50 cursor-not-allowed' : ''}`}
                                           style="resize: none; overflow: hidden; box-sizing: border-box;"
                                           placeholder={t().agentPlaceholder}
                                          value={props.prompt ? props.prompt() : ""}
                                          onInput={(e) => props.setPrompt(e.target.value)}
                                          onKeyDown={(e) => {
...
<div class="flex gap-2 overflow-visible">
                                                ...
                                                <ul class="dropdown menu w-52 rounded-box bg-base-100 shadow-sm"
                                                    popover id="mode-popover-1" style="position-anchor:--mode-anchor-1">
                                                    <For each={['General Mode', 'Accelerator Mode', 'Services Mode', 'Venture Mode', 'Studio Mode']}>
                                                        {(mode) => (
                                                            <li>
                                                                <button type="button" class="flex items-center flex gap-3 gap-2 px-3 py-2 hover:bg-base-200 w-full text-left text-base-content/50 cursor-pointer" onClick={(e) => { e.preventDefault(); setSelectedMode(mode); }}>
                                  ...
                                  <div class="flex gap-2">
                                      ...
                                      <button type="button" style="background-color: #00a7e0;" class="text-white px-2 py-1 rounded-full flex items-center text-xs transition group" onClick={() => props.handleInstruct && props.handleInstruct()}>
                                          <i data-lucide="book-open" class="w-3 h-4"></i>
                                      ...
                                      <button type="button" style="background-color: #ffc600;" class="text-white px-2 py-1 rounded-full flex items-center text-xs transition group" onClick={() => props.handleRegenerate && props.handleRegenerate()}>
                                          <i data-lucide="refresh-ccw" class="w-3 h-4"></i>
                                      ...
                                      {/* State-based buttons */}
                                      {/* <Show when={uiState() === 'idle'}>
<button type="button"
                                                onClick={(e) => props.handleStart && props.handleStart()}
                                      ...
                                      <Show when={uiState() === 'confirm'}>
                                          <div class="flex gap-1">
                                              <button type="button"
                                                  onClick={(e) => props.handleConfirmAccept && props.handleConfirmAccept()}
...
<i data-lucide="check" class="w-3 h-4"></i>
                                              ...
                                              <button type="button"
                                                  onClick={(e) => props.handleConfirmRetry && props.handleConfirmRetry()}
                                              ...
                                              <button type="button"
                                                  onClick={(e) => props.handleConfirmEdit && props.handleConfirmEdit()}
                                              ...
                                              <button type="button"
                                                  onClick={(e) => props.handleConfirmReset && props.handleConfirmReset()}
                                      ...
                                      <Show when={isComplete()}>
                                          <button type="button"
                                              onClick={(e) => props.handleReset && props.handleReset()}
