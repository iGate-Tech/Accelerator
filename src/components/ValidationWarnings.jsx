...
const ValidationWarnings = (props) => {
  ...
  return (
    <Show when={hasIssues()}>
      <div class={`alert ${hasErrors() ? 'alert-error' : 'alert-warning'} mb-2 mt-2`}>
        <svg xmlns="http://www.w3.org/2000/svg" class="stroke-current shrink-0 h-5 w-5" fill="none" viewBox="0 0 24 24">
          <Show when={hasErrors()} fallback={
              ...
              <For each={allIssues()}>
                {(issue) => (
                  <div class="flex items-center gap-1">
                    <span class={`badge badge-xs ${issue.type === 'error' ? 'badge-error' : 'badge-warning'} shrink-0`}>
                      {issue.type === 'error' ? '✕' : '!'}
                    </span>
