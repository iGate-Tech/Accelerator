...
class MetricsService {
  ...
  async trackUserActivity(userId, action) {
    ...
    let dauSet = (await cacheService.get(key)) || [];
    ...
    let actionCount = (await cacheService.get(actionKey)) || 0;
    await cacheService.set(actionKey, actionCount + 1, 86400);

    // Track user actions
    const userActionKey = `metrics:user:${userId}:${action}:${date}`;
    let userActionCount = (await cacheService.get(userActionKey)) || 0;
  ...
  async trackIdeaCompletion(userId, ideaId, completionRate) {
    ...
    let count = (await cacheService.get(`${key}:count`)) || 0;
    await cacheService.set(`${key}:count`, count + 1, 86400);

    // Add to completion rate sum
    let rateSum = (await cacheService.get(`${key}:rate_sum`)) || 0;
    ...
    let userCount = (await cacheService.get(userKey)) || 0;
  ...
  async trackReportGeneration(userId, reportType) {
    ...
    let total = (await cacheService.get(`${key}:total`)) || 0;
    await cacheService.set(`${key}:total`, total + 1, 86400);

    let reportTypeCount = (await cacheService.get(`${key}:${reportType}`)) || 0;
  ...
  async trackCreditUsage(userId, amount, type) {
    ...
    let used = (await cacheService.get(`metrics:credits:used:${date}`)) || 0;
    await cacheService.set(
  ...
  async trackVote(userId, ideaId) {
    ...
    let totalVotes = (await cacheService.get(`metrics:votes:${date}`)) || 0;
    await cacheService.set(`metrics:votes:${date}`, totalVotes + 1, 86400);

    let userVotes =
      (await cacheService.get(`metrics:user:${userId}:votes:${date}`)) || 0;
    await cacheService.set(
      `metrics:user:${userId}:votes:${date}`,
      userVotes + 1,
      86400,
    );

    let voters = (await cacheService.get(`metrics:voters:${date}`)) || [];
    if (!voters.includes(userId)) {
      voters.push(userId);
      await cacheService.set(`metrics:voters:${date}`, voters, 86400);
    }
  }
