...
const ProtectedRoute = (props) => {
  ...
  logger.debug('ProtectedRoute: Authentication check - isAuthenticated:', auth, 'current path:', window.location.pathname);

  // If not authenticated, redirect to login
  if (!auth) {
    logger.info('ProtectedRoute: User not authenticated, redirecting to /auth/login');
