...
router.get("/api/ideas", requireAuth, async (req, res) => {
  try {
    ...
    const { data, error } = await supabase.rpc("get_user_ideas", {
    ...
    if (error) {
      ...
      return res
        .status(500)
        .json({ success: false, error: "Failed to fetch ideas" });
    }
...
router.post("/api/ideas", requireAuth, async (req, res) => {
  try {
    ...
    // Use DB function for validation and creation
    const { data, error } = await supabase.rpc("validate_and_create_idea", {
      p_user_id: userId,
      p_title: title,
      p_description: description,
      p_category: category || "Other",
      p_tags: Array.isArray(tags) ? tags : [],
      p_privacy: privacy || "private",
    });
    ...
    if (error) {
      ...
      if (req.isHtmx) {
        return res.send(`
          <div data-slot="alert" role="alert" class="w-full rounded-lg border px-4 py-3 text-sm alert-destructive">
    ...
    if (!data.success) {
      ...
      if (req.isHtmx) {
        const errorMsg = data.errors[0] || "Validation failed";
        const alertType = errorMsg.includes("Title")
...
router.get("/api/ideas/:id", requireAuth, async (req, res) => {
  try {
    ...
    // Get comprehensive idea details using database function
    const { data: ideaResult, error } = await supabase.rpc("comprehensive_idea_operations", {
      p_user_id: userId,
      p_action: "get_details",
      p_data: json_build_object('idea_id', id)
    });

    if (error || !ideaResult?.success) {
      if (ideaResult?.error === 'Idea not found') {
        return res.status(404).json({
          success: false,
          error: "Idea not found",
        });
      }
      logger.error("Error fetching idea:", error || ideaResult?.error);
      return res.status(500).json({
        success: false,
    ...
    res.json({
      ...
      permissions: ideaResult.permissions
    });
      }
...
router.put("/api/ideas/:id", requireAuth, async (req, res) => {
  try {
    ...
    if (activityError) {
      logger.error("Failed to log idea update activity:", activityError);
    }
...
router.delete("/api/ideas/:id", requireAuth, async (req, res) => {
  try {
    ...
    if (activityError) {
      logger.error("Failed to log idea deletion activity:", activityError);
    }
...
router.post("/api/ideas/:id/favorite", requireAuth, async (req, res) => {
  try {
    ...
    const { error: activityError } = await supabase.rpc("log_user_activity", {
      p_user_id: userId,
      p_action_type: isFavorite ? "favorite" : "unfavorite",
      p_entity_type: "idea",
      p_entity_id: id,
      p_details: {
        title: idea.title,
    ...
    if (activityError) {
      logger.error("Failed to log favorite activity:", activityError);
    }
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      // Create the idea using database function
      const { data: createResult, error } = await supabase.rpc(
        "validate_and_create_idea",
        {
          p_user_id: userId,
          p_title: title.trim(),
      ...
      if (error || !createResult?.success) {
        logger.error("Error creating idea:", error || createResult?.error);
        if (req.isHtmx) {
          return res.send(`
           <div data-slot="alert" role="alert" class="w-full rounded-lg border px-4 py-3 text-sm alert-destructive grid has-[>svg]:grid-cols-[calc(var(--spacing)*4)_1fr] grid-cols-[0_1fr] has-[>svg]:gap-x-3 gap-y-0.5 items-start [&>svg]:size-4 [&>svg]:translate-y-0.5 [&>svg]:text-current alert-auto-hide alert-sticky">
             <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-circle-alert">
             ...
             <div data-slot="alert-description" class="text-muted-foreground col-start-2 grid justify-items-start gap-1 text-sm [&_p]:leading-relaxed">${createResult?.error || "Failed to create idea. Please try again."}</div>
           </div>
           <script>hideLoading();</script>
         `);
        }
        ...
        return res.send(`
          <div data-slot="alert" role="alert" class="w-full rounded-lg border px-4 py-3 text-sm alert-success grid has-[>svg]:grid-cols-[calc(var(--spacing)*4)_1fr] grid-cols-[0_1fr] has-[>svg]:gap-x-3 gap-y-0.5 items-start [&>svg]:size-4 [&>svg]:translate-y-0.5 [&>svg]:text-current alert-auto-hide alert-sticky">
           ...
           <script>
             ...
             setTimeout(() => {
               window.location.href = '/models/idea?ideaId=${createResult.idea_id}';
             }, 1000);
           </script>
         `);
      }
      res.redirect(`/models/idea?ideaId=${createResult.idea_id}`);
    } catch (error) {
      logger.error("Create idea error:", error);
      res.redirect("back");
    }
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (activityError) {
        logger.error(
          "Failed to log random idea generation activity:",
          activityError,
        );
      }
