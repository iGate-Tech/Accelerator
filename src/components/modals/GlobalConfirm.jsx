      ...
      <div class="modal modal-open">
        <div class="modal-box relative">
          ...
          <div class="flex items-start gap-4">
            <div
              class={`rounded-full p-3 ${confirmState().type === 'danger' ? 'bg-error/20' : 'bg-warning/20'}`}
            >
              <div
                class={`h-6 w-6 ${confirmState().type === 'danger' ? 'text-error' : 'text-warning'}`}
                innerHTML={getIconSvg(
                  confirmState().icon || 'alert-triangle',
                  ''
                )}
              />
            </div>
            ...
            <div class="flex-1">
              <h3 class="mb-2 text-lg font-bold">{confirmState().title}</h3>
        ...
        <div class="modal-backdrop bg-black/50" onClick={handleCancel} />
      </div>
    </Show>
  );
};
