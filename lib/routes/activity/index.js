...
router.get("/summary", requireAuth, async (req, res) => {
  try {
    ...
    activities?.forEach((activity) => {
      ...
      // By action type
      if (!summary.by_type[activity.action_type]) {
        summary.by_type[activity.action_type] = 0;
      }
      summary.by_type[activity.action_type]++;

      // By entity type
      if (!summary.by_entity[activity.entity_type]) {
        summary.by_entity[activity.entity_type] = 0;
      }
      summary.by_entity[activity.entity_type]++;
    });

    // Convert to array for easier consumption
    summary.daily_activity = Object.entries(summary.by_date)
