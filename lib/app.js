...
// Custom profile detector for i18next
const profileDetector = {
  name: "profile",
  lookup: (req) => req.user?.profile?.preferences?.language,
};
...
app.engine(
  ...
  exphbs.engine({
    ...
    helpers: { ...helpers, t: (key, options) => i18next.t(key, options.hash) },
  }),
);
...
app.use((req, res, next) => {
  console.log(`i18n: Detected language: ${req.language}, Session: ${req.session?.language}, Profile: ${req.user?.profile?.preferences?.language}`);
  next();
});
