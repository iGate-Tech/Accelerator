...
router.post("/", requireAuth, async (req, res) => {
  try {
    ...
    if (voteError) {
      logger.error("Vote upsert error:", voteError);
    ...
    if (updatedVotes && updatedVotes.length > 0) {
      const averageRating =
        updatedVotes.reduce((sum, vote) => sum + vote.rating, 0) /
        updatedVotes.length;
      await supabase
        .from("ideas")
    ...
    try {
      await checkVotingMilestones(supabase, idea_id, idea.title, idea.user_id);
    } catch (error) {
    ...
    try {
      await checkValidationThreshold(supabase, idea_id, idea.title);
    } catch (error) {
...
async function checkVotingMilestones(supabase, ideaId) {
  try {
    // Get vote count
...
async function checkValidationThreshold(supabase, ideaId, ideaTitle) {
  try {
