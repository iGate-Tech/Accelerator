...
router.post("/auth/signup", async (req, res) => {
  ...
  try {
    ...
    // Update profile with name from user metadata using database function
    const name =
      `${data.user.user_metadata?.firstName || ""} ${data.user.user_metadata?.lastName || ""}`.trim();
    if (name) {
      const { data: profileUpdateResult, error: profileUpdateError } =
        await supabaseAdmin.rpc("update_user_profile_safe", {
          p_user_id: data.user.id,
          p_updates: { name },
        });
      if (profileUpdateError) {
        logger.error("Failed to update user profile name:", profileUpdateError);
      }
    }

    // Log signup activity using database function
    const { error: activityError } = await supabaseAdmin.rpc(
      "log_user_activity",
      {
        p_user_id: data.user.id,
    ...
    if (activityError) {
      logger.error("Failed to log signup activity:", activityError);
    }

    // Create welcome notifications using database function
    const { error: welcomeError } = await supabaseAdmin.rpc(
    ...
    if (welcomeError) {
      logger.error("Failed to create welcome notification:", welcomeError);
    }

    const { error: gettingStartedError } = await supabaseAdmin.rpc(
      "create_user_notification",
      {
        p_user_id: data.user.id,
        p_type: "getting_started",
        p_message:
    ...
    if (gettingStartedError) {
      logger.error(
        "Failed to create getting started notification:",
        gettingStartedError,
      );
    }

    // Create tip notification using database function
    const { error: tipError } = await supabaseAdmin.rpc(
      "create_user_notification",
      {
        p_user_id: data.user.id,
        p_type: "tip",
    ...
    if (tipError) {
      logger.error("Failed to create tip notification:", tipError);
    }
