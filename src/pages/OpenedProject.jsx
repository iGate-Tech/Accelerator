...
import { steps, stepNames, buildPrompt, createStepHook } from "@lib/business.js";
...
const OpenedProject = (props) => {
    ...
    const [currentProjectId, setCurrentProjectId] = createSignal(params.id || projectsStore.currentProjectId);
    ...
    const getRemainingTasks = (projectTasks) => {
      ...
      for (const task of projectTasks) {
        ...
        const hasValidContent = content.length > 20 && !isPlaceholderContent(content);
        const hasValidLlmResponse = llmResponse.length > 20 && !isPlaceholderContent(llmResponse);
        ...
        if (isCompleted) {
          ...
          console.log(`Task marked as completed: ${task.title || task.stepName || task.step || 'Unknown'} - Content length: ${content.length}, LLM response length: ${llmResponse.length}`);
        } else {
          console.log(`Task marked as incomplete: ${task.title || task.stepName || task.step || 'Unknown'} - Content length: ${content.length}, LLM response length: ${llmResponse.length}`);
        }
      }

      // Find steps that are not completed yet
      const remainingSteps = [];
      console.log(`Total steps to check: ${steps.length}, Completed steps set:`, completedSteps);

      for (let i = 0; i < steps.length; i++) {
        const step = steps[i];
        const stepId = step.id.toLowerCase();
        const stepNameEn = (stepNames('en')[step.id] || step.name || '').toLowerCase();
            ...
            if (currentProjectId() && !startPressed()) {
                ...
                try {
                    ...
                    if (project.state === 'pending') {
                        // Wait a bit more for the project to load
                    ...
                    if (projectTasks.length === 0) {
                        ...
                        console.log(`Project has ${projectTasks.length} total tasks, ${remainingTasks.length} remaining`);
                        ...
                        if (remainingTasks.length > 0) {
                            ...
                            console.log(`Project has ${remainingTasks.length} remaining tasks`);
                        } else {
                    ...
                    try {
                        ...
                        if (projectTasks.length === 0) {
                            ...
                            const remainingTasks = getRemainingTasks(projectTasks);
                            console.log(`After error, project has ${remainingTasks.length} remaining tasks`);
                        }
                    } catch (taskError) {
    ...
    const [tasks, { refetch: refetchTasks, mutate }] = createResource(currentProjectId, async (projectId) => {
        if (!projectId) return [];
        try {
            const result = await getTasks(String(projectId));
            console.log(`Loaded ${result?.length || 0} tasks for project ${projectId}`);
            return result || [];
        } catch (error) {
...
Please provide the modified content that follows the instruction.`;
    ...
    const handleConfirm = async (taskId) => {
      console.log('[Journey] ============================================');
      console.log('[Journey] STEP CONFIRM: User confirmed task, determining next step to process');
      ...
      try {
        ...
        if (remainingSteps.length > 0) {
          ...
          const stepName = stepNames(currentLang())[nextStepInfo.id] || nextStepInfo.name || `Step ${nextStepInfo.index + 1}`;
          ...
          const taskPrompt = buildPrompt(nextStepInfo.id, { problem: prompt(), lang: currentLang() });
          ...
          console.log('[Journey] Created new task for next step', taskId, 'with prompt length:', taskPrompt?.length || 0);
          ...
          try {
            ...
            if (stillRemaining.length > 0) {
              toastManager.info(`${stillRemaining.length} more tasks remaining...`);
            } else {
    ...
    const handleStart = async () => {
      ...
      try {
        ...
        // Create new task
        const userId = user()?.id || 'unknown_user'; // Use a fallback if user is not available
        const taskPrompt = buildPrompt(stepInfo.id, { problem: prompt(), lang: currentLang() });
    ...
    const refreshTasks = async () => {
      ...
      console.log(`RefreshTasks: Loaded ${tasksData?.length || 0} tasks for project ${currentProjectId()}`);
    ...
    return (
      <div class="flex flex-col h-screen max-w-full lg:max-w-6xl mx-auto bg-base-100">
