...
preloadIcons.forEach((iconName) => {
  ...
  if (Object.hasOwn(lucideStatic, capitalizedName)) {
...
export default {
  ...
  lookup: (obj, key) => (obj && Object.hasOwn(obj, key) ? obj[key] : undefined),
  ...
  icon: function (name, options) {
    ...
    const { class: className = "", size = 24, fill, stroke, id } = attrs;
    ...
    if (className) {
      ...
      // If class contains fill-, change fill to currentColor
