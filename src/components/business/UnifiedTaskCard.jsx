import { createMemo, createEffect, Show, createSignal, useContext } from 'solid-js';
  ...
  createEffect(() => {
    ...
    if (
      ...
      requestAnimationFrame(() => {
        ...
        // First, get all the measurements we need (reads)
        const rect = cardRef.getBoundingClientRect();
        const currentScrollY = window.scrollY || window.pageYOffset;

        // Then perform all the DOM manipulations (writes)
      ...
      >
        ...
        <div class="flex items-center gap-2">
          <Show when={isSelected()}>
            <span class="badge badge-info badge-sm">{t().selected}</span>
          </Show>
          <Show
            when={
              props.streamingTaskId?.() === taskId &&
              ...
              >
                ...
                >
                  <div class="text-base-content/60 flex items-center gap-2">
                    ...
                    <span class="text-sm">{t().generatingResponse}</span>
                  </div>
                </Show>
                <Show
                  when={
                ...
                >
                  <div class="text-base-content/60 flex items-center gap-2">
                    <div class="loading loading-dots loading-sm" />
                    <span class="text-sm">{t().waitingResponse}</span>
                  </div>
                </Show>
              </div>
            }
            ...
            <div class="space-y-3">
              <div class="flex items-center justify-between">
                <label class="label py-0">
                  <span class="label-text font-medium">{t().editContent}</span>
                </label>
