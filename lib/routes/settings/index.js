    ...
    if (req.files && req.files.avatar) {
      ...
      if (!avatarFile.mimetype.startsWith("image/")) {
        ...
        if (uploadError) {
          ...
          // Activity logging handled automatically by triggers
        }
      }
    }
...
router.post("/payment-methods", requireAuth, async (req, res) => {
  try {
    ...
    // Save payment method settings using database function
...
router.post("/settings/theme", requireAuth, async (req, res) => {
  try {
    ...
    if (error) {
      ...
      // Update theme setting using database function
      ...
      if (settingsError) {
        logger.error("Error updating theme setting:", settingsError);
...
router.post("/settings/language", requireAuth, async (req, res) => {
  try {
    ...
    if (error) {
      ...
      // Update language setting using database function
      const { error: settingsError } = await supabase.rpc(
        "manage_user_settings",
        {
          p_user_id: req.user.id,
      ...
      if (settingsError) {
        logger.error("Error updating language setting:", settingsError);
