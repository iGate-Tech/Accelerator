...
router.get("/user/stats", requireAuth, async (req, res) => {
  try {
    ...
    console.error("Get user voting stats error:", error);
    res.status(500).json({
      success: false,
      error: "Internal server error",
    });
...
async function checkVotingMilestones(ideaId, ideaTitle, ideaOwnerId) {
  try {
    // Get vote count
    ...
    for (const milestone of milestones) {
      if (voteCount === milestone) {
        // Send milestone email to idea owner
        try {
          const { sendVotingThresholdEmail } = await import(
            "../../services/email.js"
          );
          const { data: ownerAuth } =
            await supabase.auth.admin.getUserById(ideaOwnerId);
          if (ownerAuth.user?.email) {
            ...
            const nextThreshold =
              milestones[milestones.indexOf(milestone) + 1] || milestone * 2;
            await sendVotingThresholdEmail(ownerAuth.user.email, firstName, {
              ideaTitle,
              votes: milestone,
              nextThreshold,
              rewardCredits: Math.floor(milestone / 10), // Reward based on milestone
              ideaId,
            });
          }
        } catch (emailError) {
    ...
    console.error("Error in checkVotingMilestones:", error);
  }
}
...
async function checkValidationThreshold(ideaId, ideaTitle) {
  try {
    ...
    if (averageRating >= 3) {
      ...
      // Send validation email notification to idea owner
      try {
        const { sendIdeaValidatedEmail } = await import(
          "../../services/email.js"
        );
        ...
        if (idea) {
          ...
          if (userAuth.user?.email) {
            ...
            await sendIdeaValidatedEmail(userAuth.user.email, firstName, {
              title: idea.title,
            });
          }
        }
        ...
        // Don't fail the voting if email fails
      }
      ...
      if (!existingRewards || existingRewards.length === 0) {
        ...
        for (const vote of ideaVotes) {
          ...
          // Send reward distribution email
          try {
            const { sendRewardDistributionEmail } = await import(
              "../../services/email.js"
            );
            const { data: voterAuth } = await supabase.auth.admin.getUserById(
              vote.user_id,
            );
            if (voterAuth.user?.email) {
              const firstName = voterProfile?.name?.split(" ")[0] || "there";
              await sendRewardDistributionEmail(
                voterAuth.user.email,
                firstName,
                {
                  totalCredits: rewardPerVoter,
                  newBalance: voterProfile
                    ? voterProfile.credit_balance + rewardPerVoter
                    : rewardPerVoter,
                  breakdown: [
                    {
                      description: `Voting on "${ideaTitle}"`,
                      credits: rewardPerVoter,
                    },
                  ],
                },
              );
        ...
        console.log(
          `Distributed ${rewardPerVoter} credits each to ${ideaVotes.length} voters for idea ${ideaId}`,
        );
      }
    }
  } catch (error) {
    console.error("Error in checkValidationThreshold:", error);
  }
}
