...
router.post("/auth/signup", async (req, res) => {
  ...
  try {
    ...
    if (!terms) {
      ...
      if (req.isHtmx) {
        return res.send(
          `<script>showToast('You must accept the Terms of Service and Privacy Policy.', 'error');</script>`,
        );
      }
    ...
    if (!data.session) {
      ...
      if (req.isHtmx) {
        return res.send(
          `<script>showToast('Account created! Please check your email to confirm your account.', 'info'); setTimeout(() => window.location.href = '/auth/login', 1000);</script>`,
        );
      }
