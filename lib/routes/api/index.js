...
router.get("/ideas", requireAuth, async (req, res) => {
  try {
    const supabase = getSupabaseClient(req);
    const { type, limit = 20, offset = 0, search, category } = req.query;
...
router.get("/models/:id/sections/:section", requireAuth, async (req, res) => {
  try {
    ...
    console.error("Get model section API error:", error);
    res.status(500).json({
      success: false,
      error: "Internal server error",
    });
...
router.put("/models/:id/sections/:section", requireAuth, async (req, res) => {
  try {
    ...
    console.error("Update model section API error:", error);
    res.status(500).json({
      success: false,
      error: "Internal server error",
    });
...
async function checkIdeaValidation(supabase, ideaId) {
  ...
  if (!votes || votes.length < 3) return;
...
router.post("/set-language", async (req, res) => {
  try {
    ...
    if (req.user) {
      ...
      if (error) {
        console.warn("Failed to update profile language:", error.message);
