...
router.post('/api/models/:type', requireAuth, async (req, res) => {
  try {
    ...
    if (error) {
      logger.error('Error creating model instance:', error);
  ...
  async (req, res) => {
    try {
      ...
      if (sectionError && sectionError.code !== 'PGRST116') {
        logger.error('Error fetching section:', sectionError);
        return res.status(500).json({
          success: false,
      ...
      logger.error('Get section API error:', error);
      res.status(500).json({
        success: false,
        error: 'Internal server error',
      });
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      logger.error('Update section API error:', error);
      res.status(500).json({
        success: false,
        error: 'Internal server error',
      });
...
async function updateModelCompletion(modelId, supabase) {
  try {
    ...
    if (error || !sections) {
      logger.error('Error fetching sections for completion:', error);
      return;
    }
    ...
    logger.error('Error updating model completion:', error);
  }
}
...
router.post('/models/:modelType', requireAuth, async (req, res) => {
  try {
    ...
    if (instanceError && instanceError.code === 'PGRST116') {
      ...
      if (createError) {
        logger.error('Error creating model instance:', createError);
        return res.redirect('back');
      }
      modelInstance = newInstance;
    } else if (instanceError) {
      logger.error('Error finding model instance:', instanceError);
      return res.redirect('back');
    }
    ...
    if (saveError) {
      logger.error('Error saving model section:', saveError);
      return res.redirect('back');
    }
    ...
    logger.error('Model section save error:', error);
    res.redirect('back');
  }
});
