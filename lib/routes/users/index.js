...
router.post('/profile', requireAuth, async (req, res) => {
  try {
    ...
    if (package_type !== undefined) {
      updateData.package_type = package_type;
    }
...
router.post('/profile/avatar', requireAuth, async (req, res) => {
  try {
    ...
    const filePath = `avatar/${Date.now()}-${userId}-${avatarFile.name}`;
    ...
    if (error) {
      logger.error('Error updating profile with avatar:', error);
      return res.status(500).json({
        success: false,
...
router.get('/credits', requireAuth, async (req, res) => {
  try {
    ...
    if (profileError) {
      logger.error('Error fetching credits:', profileError);
      return res.status(500).json({
        success: false,
