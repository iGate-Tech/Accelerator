...
router.put("/ideas/:id/archive", requireAuth, async (req, res) => {
  try {
    const supabase = getSupabaseClient(req);
    const { id } = req.params;
    ...
    const { data: idea, error } = await supabase
      .from("ideas")
      .update({ overall_status: "archived" })
      .eq("id", id)
      .select()
      .single();
    ...
    logger.error("Archive idea API error:", error);
    res.status(500).json({
      success: false,
      error: "Internal server error",
    });
...
// PUT /api/ideas/:id/favorite
router.put("/ideas/:id/favorite", requireAuth, async (req, res) => {
  try {
    const supabase = getSupabaseClient(req);
    const { id } = req.params;
    ...
    // Check if user can favorite (public ideas or own ideas)
    ...
    // Toggle favorite
    const newFavorite = !existingIdea.is_favorite;
    ...
    res.json({
      ...
      message: newFavorite ? "Idea favorited" : "Idea unfavorited",
      idea,
    });
  } catch (error) {
    logger.error("Favorite idea API error:", error);
    res.status(500).json({
      success: false,
      error: "Internal server error",
    });
...
// PUT /api/ideas/:id/unpublish
router.put("/ideas/:id/unpublish", requireAuth, async (req, res) => {
  try {
    const supabase = getSupabaseClient(req);
    const { id } = req.params;
    ...
    logger.error("Unpublish idea API error:", error);
    res.status(500).json({
      success: false,
      error: "Internal server error",
    });
...
router.post("/reports/:type", requireAuth, async (req, res) => {
  try {
    ...
    // Check credit balance
    const { balance: currentBalance } = await getUserCredits(req.user.id);
...
router.get("/credits/balance", requireAuth, async (req, res) => {
  try {
    const { balance } = await getUserCredits(req.user.id);
    res.send(`${balance} Credits`);
  } catch (error) {
    logger.error("Get credits balance error:", error);
    res.status(500).send("Error loading credits");
  }
});
