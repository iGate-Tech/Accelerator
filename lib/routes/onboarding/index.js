...
router.post("/onboarding/plan", requireAuth, async (req, res) => {
  try {
    ...
    // Determine next step based on package type
    const nextStep =
      package_type === "free" ? "/onboarding/profile" : "/onboarding/payment";
...
router.post("/onboarding/payment", requireAuth, async (req, res) => {
  try {
    ...
    // If free package, skip payment and go to profile
      ...
      if (req.isHtmx) {
        return res.send(`
          <div data-slot="alert" role="alert" class="w-full rounded-lg border px-4 py-3 text-sm alert-success grid has-[>svg]:grid-cols-[calc(var(--spacing)*4)_1fr] grid-cols-[0_1fr] has-[>svg]:gap-x-3 gap-y-0.5 items-start [&>svg]:size-4 [&>svg]:translate-y-0.5 [&>svg]:text-current alert-auto-hide alert-sticky">
            ...
            <div data-slot="alert-title" class="col-start-2 line-clamp-1 min-h-4 font-medium tracking-tight">Free Plan Activated</div>
            <div data-slot="alert-description" class="text-muted-foreground col-start-2 grid justify-items-start gap-1 text-sm [&_p]:leading-relaxed">No payment required for free plan.</div>
          </div>
          <script>
            // Small delay to ensure alert is shown
            setTimeout(() => {
...
router.post("/onboarding/profile", requireAuth, async (req, res) => {
  try {
    ...
    if (name.trim().length > 100) {
      const errorMsg = "Name must be less than 100 characters.";
      if (req.isHtmx) {
        return res.send(`
          <div data-slot="alert" role="alert" class="w-full rounded-lg border px-4 py-3 text-sm alert-destructive grid has-[>svg]:grid-cols-[calc(var(--spacing)*4)_1fr] grid-cols-[0_1fr] has-[>svg]:gap-x-3 gap-y-0.5 items-start [&>svg]:size-4 [&>svg]:translate-y-0.5 [&>svg]:text-current alert-auto-hide alert-sticky">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-circle-alert">
      ...
      return res.redirect("/onboarding/profile?error=name_too_long");
    }
    ...
    if (bio && bio.length > 500) {
      ...
      return res.redirect("/onboarding/profile?error=bio_too_long");
    }

    // Validate location length
    if (location && location.length > 100) {
      const errorMsg = "Location must be less than 100 characters.";
      if (req.isHtmx) {
        return res.send(`
          <div data-slot="alert" role="alert" class="w-full rounded-lg border px-4 py-3 text-sm alert-destructive grid has-[>svg]:grid-cols-[calc(var(--spacing)*4)_1fr] grid-cols-[0_1fr] has-[>svg]:gap-x-3 gap-y-0.5 items-start [&>svg]:size-4 [&>svg]:translate-y-0.5 [&>svg]:text-current alert-auto-hide alert-sticky">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-circle-alert">
      ...
      return res.redirect("/onboarding/profile?error=location_too_long");
    }
    ...
    // Create profile completion notification using database function
    ...
    if (notificationError) {
      logger.error(
        "Failed to create profile completion notification:",
        notificationError,
      );
    }
