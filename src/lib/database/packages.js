...
export async function _createUserSubscription({
  userId,
  packageId,
  subscriptionData = {},
}) {
...
export async function _changeUserSubscription({
  userId,
  newPackageId,
  currentSubscription,
}) {
  try {
    ...
    if (currentSubscription) {
      await dbInstance.query(
        'UPDATE user_subscriptions SET status = $1, cancelled_at = $2 WHERE id = $3',
        ['cancelled', new Date().toISOString(), currentSubscription.id]
      );
    }
    ...
    const result = await _createUserSubscription({
      userId,
...
export async function _updateUserSubscription({
  userId,
  subscriptionId,
  updates,
}) {
