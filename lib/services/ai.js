...
export async function testProvider(provider = null) {
  try {
    const systemPrompt =
      "Test the AI provider connectivity. Respond with a simple confirmation.";
    const maxTokens = 50;
    const temperature = 0.7;
    const result = await callAI(
      "", // No user prompt, system prompt contains everything
      systemPrompt,
      { maxTokens, temperature, provider },
    );

    // Note: Test provider doesn't deduct credits
...
async function callOpenRouter(prompt, systemPrompt, options = {}) {
  try {
    ...
    if (
      systemPrompt &&
      config.ai.openrouter.model !== "google/gemma-3n-e2b-it:free"
    ) {
    ...
    if (result.success) {
      ...
      if (balance < 10) {
        return {
          success: false,
          error: new Error(
            "Insufficient credits. Content improvement costs 10 credits.",
          ),
        };
      }
      ...
      if (error || !data.success) {
        throw new Error(
          data?.error || error?.message || "Credit deduction failed",
        );
      }
    }
 ...
 */
export async function suggestAutoFill(formData, userId) {
  ...
  const missingFields = [];
  if (
    !existingFields.includes("title") ||
    !existingData.title ||
    existingData.title.toString().trim().length < 10
  ) {
    missingFields.push("title");
  }
  if (
    !existingFields.includes("category") ||
    !existingData.category ||
    existingData.category.toString().trim().length < 10
  ) {
    missingFields.push("category");
  }
  if (
    !existingFields.includes("description") ||
    !existingData.description ||
    existingData.description.toString().trim().length < 10
  ) {
    missingFields.push("description");
  }
  if (
    !existingFields.includes("tags") ||
    !existingData.tags ||
    existingData.tags.toString().trim().length < 10
  ) {
  ...
  missingFields.forEach((field) => {
    if (field === "title") {
      prompt += `    "title": "suggested title here",\n`;
    }
    if (field === "category") {
...
Focus on creating cohesive, professional startup ideas that build naturally from the provided information.`;
  ...
  try {
    ...
    Object.entries(parsed.suggestions || {}).forEach(([key, value]) => {
    ...
    if (balance < 8) {
      return {
        success: false,
        error: new Error("Insufficient credits. Auto-fill costs 8 credits."),
      };
    }
...
${ideaHistoryContext || "No project context provided yet"}`;
  ...
  if (config.ai.openrouter.model !== "google/gemma-3n-e2b-it:free") {
    // Use system message for models that support it
    ...
    const firstUserMessage = openRouterMessages.find(
      (msg) => msg.role === "user",
    );
    if (firstUserMessage) {
