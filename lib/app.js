...
// Load all translations from separate files (excluding translation.json to avoid conflicts)
function loadTranslations(lang) {
  ...
  try {
    ...
    for (const file of files) {
      // Skip the main translation.json file since all translations are now in separate files
      if (file.endsWith('.json') && file !== 'translation.json') {
    ...
    console.log(`Loaded ${lang} translations with ${totalKeys} keys from ${files.length - 1} separate files`);
  } catch (error) {
...
i18next.use(i18nextMiddleware.LanguageDetector).init({
  ...
  supportedLngs: ["en", "ar"],
  ns: allNamespaces,
  defaultNS: allNamespaces[0] || "sidebar", // Use first available namespace as default
  detection: {
